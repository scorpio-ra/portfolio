...
function startdraw(id) {
    ...
    // Определяем функции для рисования

    // Функция выполняется при нажатии
    function penPress(e) {
        var posX=null;
        var posY=null;
        ...
        x = posX + (html && html.scrollLeft || body && body.scrollLeft || 0) - (html.clientLeft || 0)-coord.left;
        y = posY + (html && html.scrollTop || body && body.scrollTop || 0) - (html.clientTop || 0)-coord.top;
    ...
    // Функция должна выполняться при отпускании мыши или пальца
    function penUp(e) {
        x = null;
        y = null;
        ctx.closePath();
    ...
    // Функция выполняется при движении нажатой мыши или пальца
    function penMove(e) {
    ...
    canv1.addEventListener('mousedown',penPress);
    document.addEventListener('mouseup',penUp);
    document.addEventListener('mousemove',penMove);
    canv1.addEventListener("touchstart",penPress,{passive: false});
    document.addEventListener("touchend",penUp,{passive: false});
    document.addEventListener("touchmove",penMove,{passive: false});
    /*
